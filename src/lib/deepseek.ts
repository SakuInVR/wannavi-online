...
export function buildRoadmapSystemPrompt(params: RoadmapArticleInput): string {
  return [
    ...
    "   - 各ステップで具体的に何をするか、どうなればクリアかを分かりやすく記述します。",
    "   - ユーザーの「1日の学習時間」を考慮した無理のないペース配分を提示します。",
    ...
    "   - 応用に役立つ信頼性の高い詳細リソース（公式ドキュメント、オープンソースプロジェクト、中級者向け書籍など）を紹介します。",
...
export function buildRoadmapUserPrompt(params: RoadmapArticleInput): string {
  ...
  return [
    ...
    "- 各ステップで、学習に使える信頼できるソース（書籍、ドキュメントなど）を具体的に挙げてください。",
