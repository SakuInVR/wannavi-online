...
export function buildArticleSystemPrompt(params: {
  ...
  if (params.aspMaterials.length > 0) {
    ...
    if (byUsage.has("step_up")) {
      structureHints.push("・記事前半で「次のステップ」として上位モデルを紹介");
    }
...
export function buildRoadmapSystemPrompt(params: RoadmapArticleInput): string {
  return [
    ...
    "",
    "【ミッション】",
    `ユーザーが掲げる目標「${params.goal}」の達成に向けた、具体的で実践的な「スモールステップ型上達ロードマップ」をMarkdown形式で作成してください。`,
    "日数のカウント（1日目、2日目など）は余計なノイズになるため使用せず、段階的な「ステップ（フェーズ）」でいかに積み重ねればなりたい自分に到達できるのかを示してください。",
    ...
    "   - このロードマップでどのようなステップを踏み、どう目標に近づくのか全体概要を提示します。",
    "2. **現在のレベルと目標のギャップ分析**",
    ...
    "4. **前半戦ロードマップ（ステップ1 〜 ステップ2）**",
    "   - 基礎スキルを身につけ、最初の一歩を踏み出すためのスモールステップ計画。",
    "   - **重要**: 各ステップ（## または ###）は必ず以下の【ステップ出力テンプレート】に完全に従って出力してください。",
    "5. **【最重要】プレミアムデリミタの挿入**",
    "   - 前半戦ロードマップ（ステップ2）の記述が終わった直後に、必ず次のデリミタを**単独行で**出力してください：",
    "     `<!-- PREMIUM_SECTION -->`",
    "   - このデリミタは、後半のプレミアム有料エリアを区切るためのシステム用タグです。前後に空行を置いてください。",
    "6. **後半戦ロードマップ（ステップ3 〜 ステップ5）**",
    "   - 基礎から応用、精度アップ、そして実践レベルへと進むための後半のロードマップ。",
    "   - 「これをやりきったら絶対に目標に到達できる」と確信できる、具体的かつハードルの高い実践的な課題（例: 小さな作品を作る、模擬案件をやってみる等）を含めます。",
    "   - **重要**: 各ステップは必ず以下の【ステップ出力テンプレート】に完全に従って出力してください。",
    "7. **挫折を防ぐための罠と対策（メンター視点）**",
    "   - 初心者がこの目標に挑む際に挫折しやすい「罠（つまずきポイント）」を解説します。以下の【挫折対策出力テンプレート】に従って出力してください。",
    "8. **目標達成への最終アクションとまとめ**",
    "   - すべてのステップをクリアした後に、次に踏み出すべきステップと、励ましのメッセージで締めます。",
    "",
    "【ステップ出力テンプレート（厳守）】",
    "各ステップ（ステップ1〜5）は、以下のフォーマットを正確にコピーして出力してください。括弧 [] は具体的な内容に書き換えてください：",
    "---",
    "### ステップ[番号]: [ステップタイトル]",
    "- **🎯 このステップのゴール**: [具体的な到達目標を1〜2文で明文化]",
    "- **⏱️ 想定学習時間**: [必要な合計学習時間やペースの目安（例: 15時間 / 1日1時間で約2週間）]",
    "- **📝 具体的なアクションプラン**:",
    "  1. [極めて具体的なアクション1（例: 〇〇サイトのアカウントを作成し初期設定する）]",
    "  2. [極めて具体的なアクション2]",
    "  3. [極めて具体的なアクション3]",
    "- **💡 つまずきやすい罠と回避策**: [このステップで初心者が直面する典型的なエラーや挫折原因と、それを回避するアプローチ]",
    "- **📚 推奨学習教材・公式ソース**:",
    "  | 教材名 / リソース名 | タイプ | 特徴・推奨理由 | 費用感 |",
    "  | :--- | :--- | :--- | :--- |",
    "  | [公式ドキュメント名や書籍名] | [公式リファレンス / 定番書籍 / 無料チュートリアルなど] | [具体的な特徴とおすすめ理由] | [無料 / ¥3,000程度 など] |",
    "  | [別のリソース名] | [タイプ] | [具体的なおすすめ理由] | [価格] |",
    "- **🏁 クリアチェックリスト**:",
    "  - [ ] [次のステップへ進むための具体的なクリア条件1]",
    "  - [ ] [次のステップへ進むための具体的なクリア条件2]",
    "---",
    "",
    "【挫折対策出力テンプレート（厳守）】",
    "「7. 挫折を防ぐための罠と対策」セクションでは、以下のフォーマットを正確に使用して2〜3個の罠を論理的・構造的に解説してください：",
    "---",
    "## 挫折を防ぐための具体的な罠と対策",
    "- **罠①：[具体的な罠のタイトル]**",
    "  - **発生メカニズム**: [なぜこの状況で挫折しそうになるのかの心理・物理的理由をロジカルに解説]",
    "  - **特効薬**: [その状態に陥らないための、あるいは抜け出すための具体的で即効性のあるアクション]",
    "- **罠②：[具体的な罠のタイトル]**",
    "  - **発生メカニズム**: [理由の解説]",
    "  - **特効薬**: [アクション]",
    "---",
...
export function buildRoadmapUserPrompt(params: RoadmapArticleInput): string {
  ...
  return [
    "以下のアンケート情報に基づいて、段階的なスモールステップ形式のロードマップ記事をMarkdown形式で作成してください。",
    "",
    ...
    "",
    "【出力条件】",
    "- 2500文字〜3500文字程度で、日数カウント（1日目、2日目...）は使わず、具体的で実践的なスモールステップ（ステップ1〜5程度）で構成してください。",
    "- 各ステップは、必ず【ステップ出力テンプレート】に完全に準拠して記述してください（マークダウン表やチェックリストも含めること）。",
    "- 挫折対策セクションは、必ず【挫折対策出力テンプレート】に完全に準拠して記述してください。",
    "- ロードマップの前半（ステップ2）が終わった後に、必ず `<!-- PREMIUM_SECTION -->` 単独行を挿入してください。",
  ].join("\n");
}
